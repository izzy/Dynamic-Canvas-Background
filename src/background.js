...
function init () {
    ...
    for (i = 0; i < nc; i = i + 1) {
        ec.save();
        tvx = tvy = 0;
        while (tvx == 0) { tvx = Math.floor(Math.random()*vx*(Math.random() <= .5?1:-1)); }
        while (tvy == 0) { tvy = Math.floor(Math.random()*vy*(Math.random() <= .5?1:-1)); }
...
function update () {
    ...
    //document.body.style.backgroundImage = 'url(' + e.toDataURL() + ')';
}
